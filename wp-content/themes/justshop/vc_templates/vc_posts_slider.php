...
while ( $my_query->have_posts() ) {
    ...
    if ( $link != 'link_no' ) {
        if ( $link == 'link_post' ) {
            $link_image_start = '<a class="link_image" href="'.get_permalink($post_id).'" title="'.sprintf( esc_attr__( 'Permalink to %s', 'templatation' ), the_title_attribute( 'echo=0' ) ).'">';
        }
    ...
    if ( $slides_content != '' && $content != '' && ( $type == ' wpb_flexslider flexslider_fade flexslider' || $type == ' wpb_flexslider flexslider_slide flexslider' ) ) {
        ...
        if ($slides_title==true) $description .= '<h2 class="post-title nt-headline">' . $link_image_start . $post_title . $link_image_end .'</h2>';
        $description .= $content;
        $description .= '</div>';
    }
...
$css_class =  apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'wpb_gallery templatation wpb_posts_slider wpb_content_element'.$el_class, $this->settings['base']);

$output .= "\n\t".'<div class="'.$css_class.'">';
$output .= "\n\t\t".'<div class="wpb_wrapper">';
$output .= wpb_widget_title(array('title' => $title, 'extraclass' => 'wpb_posts_slider_heading nt-underline'));
