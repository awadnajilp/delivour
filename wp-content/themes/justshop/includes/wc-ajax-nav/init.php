...
function yith_wcan_constructor() {
    ...
    load_plugin_textdomain( 'templatation', false, dirname( plugin_basename( __FILE__ ) ). '/languages/' );

    define( 'YITH_WCAN', true );
	if (!defined('YITH_WCAN_URL')) {
		$PLUGIN_URL = trailingslashit(get_bloginfo('template_url')) . 'includes/wc-ajax-nav/';
		define('YITH_WCAN_URL', $PLUGIN_URL);
	}
	
	if (!defined('YITH_WCAN_DIR')) {
		$PLUGIN_DIR = trailingslashit(get_template_directory()) . 'includes/wc-ajax-nav/';
		define('YITH_WCAN_DIR', $PLUGIN_DIR);
	}
...
add_action( 'after_setup_theme', 'yith_wcan_constructor' );
