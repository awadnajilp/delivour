...
class YITH_WCAN_Reset_Navigation_Widget extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array('classname' => 'yith-woo-ajax-reset-navigation yith-woo-ajax-navigation woocommerce widget_layered_nav', 'description' => __( 'Reset all filters setted by YITH WooCommerce Ajax Navigation', 'templatation') );
        $control_ops = array('width' => 400, 'height' => 350);
        parent::__construct('yith-woo-ajax-reset-navigation', __('JS WooCommerce Ajax Reset Navigation', 'templatation'), $widget_ops, $control_ops);
    }
    ...
    function widget( $args, $instance ) {
        ...
        if ( count( $_chosen_attributes ) > 0 || $min_price > 0 || $max_price > 0 ) {
            ...
            echo "<div class='yith-wcan'><a class='yith-wcan-reset-navigation button' href='{$link}'>". __( $label, 'templatation' ) ."</a></div>";
            echo $after_widget;
            echo ob_get_clean();
        } else {
            ob_end_clean();
...
function form( $instance ) {
    ...
    $defaults = array(
        'title' => '',
        'label' => __('Reset All Filters', 'templatation')
    );
    ...
    <p>
        <label>
            <strong><?php _e( 'Button Label', 'templatation' ) ?>:</strong><br />
