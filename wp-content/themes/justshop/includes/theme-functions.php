	...
	*/
	...
	/*-----------------------------------------------------------------------------------*/
	 if (!function_exists('templatation_headline_area')) {
		...
		global $woo_options, $wp_query,$wp_session;
		$tt_post_id = '';
		...
		if ( isset( $woo_options['woo_headline_default_title'] ) )  $headline_default_title = $woo_options['woo_headline_default_title']; else  $headline_default_title = 'Your favorite restaurant, delivered to home! ';
		if ( isset( $woo_options['woo_headline_default_message'] ) )  $headline_default_message = $woo_options['wofo_headline_default_message']; else  $headline_default_message = 'All orders are processed by respective restaurant. If you face any issues, Call 1800-9633-348 Now !';
		if ( isset( $woo_options['woo_header_section_layout'] ) ) $hdr_section_layout = $woo_options['woo_header_section_layout']; else  $hdr_section_layout = 'layout2';

		// fetching value from single posts .
		$single_disable_headline = get_post_meta($tt_post_id, '_single_disable_headline', true);
		$single_headline_heading = get_post_meta($tt_post_id, '_single_headline_heading', true);
		$single_headline_message = get_post_meta($tt_post_id, '_single_headline_message', true);
		...
		<section class="headline-<?php echo $tt_post_id; ?> headline-<?php echo $hdr_section_layout; ?> col-full" id="headline">
				<div class="headline-left-side">
					<div>
						<?php // awd-edited 
						$area = $_SESSION["area"];
						echo city_choose_form_headline($area);?>
					</div>						
					<div class="<?php echo $bcmp_class; ?>">  
					...
					<?php if ( is_woocommerce_activated() && ( isset($woo_options['woo_enable_catalog']) && 'false' == $woo_options['woo_enable_catalog'] ) ) {
							...
							if( isset( $woo_options['woocommerce_header_cart_link'] ) && 'true' == $woo_options['woocommerce_header_cart_link'] ) {
								global $woocommerce;  ?>
								<div class="headline-right-side"><?php justshop_cart_button(); ?></div>
						<?php	} 
						  } ?>
	...
	function templatation_logo_offset() {
		...
		if( isset( $woo_options['woo_logo_left_offset'] ) && $woo_options['woo_logo_left_offset'] <> "0" ) $logo_right_offset = $woo_options['woo_logo_left_offset'];
		if( isset( $woo_options['woo_logo_top_offset'] ) && $woo_options['woo_logo_top_offset'] <> "0" ) $logo_top_offset = $woo_options['woo_logo_top_offset'];
