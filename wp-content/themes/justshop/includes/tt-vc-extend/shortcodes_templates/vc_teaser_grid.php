...
while ( $my_query->have_posts() ) {
    ...
    if ( $grid_link != 'link_no' ) {
        $link = '<a class="more-link" href="'. get_permalink($post_id) .'"'.$link_target.' title="'. sprintf( esc_attr__( 'Permalink to %s', 'templatation' ), the_title_attribute( 'echo=0' ) ).'">'. __("Read more", "js_composer") .'</a>';
    }
    ...
    if ( $grid_link != 'link_no' ) {
        ...
        if ( $grid_link == 'link_post' ) {
            $link_image_start = '<a class="link_image" href="'.get_permalink($post_id).'"'.$link_target.' title="'.sprintf( esc_attr__( 'Permalink to %s', 'templatation' ), the_title_attribute( 'echo=0' ) ).'">';
            $link_title_start = '<a class="link_title" href="'.get_permalink($post_id).'"'.$link_target.' title="'.sprintf( esc_attr__( 'Permalink to %s', 'templatation' ), the_title_attribute( 'echo=0' ) ).'">';
        }
    ...
    if ( $grid_layout == 'title_thumbnail_text' ) {
        if ( $post_title ) 	{
            $to_filter = '<h2 class="post-title nt-headline">' . $link_title_start . $post_title . $link_title_end . '</h2>';
...
if( $grid_template == 'filtered_grid' && $teasers && !empty($teaser_categories)) {
    ...
    $categories_list = wp_list_categories(array(
        ...
        'show_option_none'   => __('No categories', 'templatation'),
        'echo' => false
    ));
    */
    ...
    $categories_list_output .= '<li class="active"><a href="#" data-filter="*">' . __('All', 'templatation') . '</a></li>';
...
$output .= "\n\t".'<div class="templatation '.$css_class.'">';
$output .= "\n\t\t".'<div class="wpb_wrapper">';
//$output .= ($title != '' ) ? "\n\t\t\t".'<h2 class="wpb_heading wpb_teaser_grid_heading">'.$title.'</h2>' : '';
$output .= wpb_widget_title(array('title' => $title, 'extraclass' => 'nt-underline wpb_teaser_grid_heading'));
