...
class Woo_AdWidget extends WP_Widget {
	...
	function Woo_AdWidget() {
		...
		parent::WP_Widget(false, __('JS - Adspace Widget', 'templatation'),$widget_ops);      
	}
...
<?php if ( current_user_can( 'unfiltered_html' ) ) : // Only show it to users who can edit it ?>
	<p>
		<label for="<?php echo $this->get_field_id('adcode'); ?>"><?php _e('Ad Code:','templatation'); ?></label>
...
<?php endif; ?>
	<p>
		<label for="<?php echo $this->get_field_id('image'); ?>"><?php _e('Image Url:','templatation'); ?></label>
	...
	<p>
		<label for="<?php echo $this->get_field_id('href'); ?>"><?php _e('Link URL:','templatation'); ?></label>
	...
	<p>
		<label for="<?php echo $this->get_field_id('alt'); ?>"><?php _e('Alt text:','templatation'); ?></label>
